...
#undef P
        ...
        if (uBytesNeeded < 256 && uBytesNeeded < uBytesToCopy)
        {
            ...
            //
            // LOCAL FIX (diverges from upstream): memmove, not memcpy. Source and destination are
            // the same buffer and overlap whenever uBytesNeeded is less than the window size,
            // which is the normal case (e.g. a 53 KB shift within a 2 MB window). memcpy is
            // undefined on overlapping ranges; it happens to produce the right bytes with
            // glibc's forward copy for these sizes, but nothing guarantees that across libc
            // implementations or CPU dispatch paths, and a silent window corruption here would
            // surface as plausible-looking garbage much later. AddressSanitizer flags the
            // original as memcpy-param-overlap.
